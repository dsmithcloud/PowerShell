...
<#
   ...
   .PARAMETER Lync
        Specifies that a connection to Lync Online should be made.
   ...
   .NOTES 
        ...
        AUTHOR: David Smith
        LASTEDIT: 01/13/2015
        KEYWORDS: Office365, Exchange Online, Lync Online, SharePoint Online, Azure AD
...
#>
    ...
    if($Exchange) {
        if ((Get-PSSession | ?{$_.ConfigurationName -eq "Microsoft.Exchange"}).State -ne "Opened") {
            ...
            $ExchangeSession = New-Pssession -ConfigurationName Microsoft.Exchange -ConnectionUri https://ps.outlook.com/powershell -Credential $Credential -Authentication Basic -AllowRedirection -sessionOption $MySessionOption
    ...
    if($Lync) {
        if ((Get-PSSession | ?{$_.ComputerName -like "*.online.lync.com"}).State -ne "Opened") {
            Write-Host "Connecting to Lync Online" -ForegroundColor Yellow
            $LyncSession = New-CsOnlineSession -Credential $Credential     -SessionOption $MySessionOption
            Import-PSSession $LyncSession
        } Else {
            Write-Host "Connection to Lync Online exists for " -NoNewline -ForegroundColor Yellow
...
function Disconnect-Office365 {
    ...
    if ($Exchange -or $Lync -or $SharePoint) {$All = $False}
    ...
    if ($All -or $Lync)       {Write-Host "Disconnecting Lync Online" -ForegroundColor Yellow; Get-PSSession | ?{$_.ComputerName -like "*lync.com"} | Remove-PSSession}
